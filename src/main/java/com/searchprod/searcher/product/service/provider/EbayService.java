...
public class EbayService {
    ...
    public Mono<ProductDetail> getProductDetail(String id, ProductIdType idType) {
        ...
        try {
            UriComponentsBuilder builder = UriComponentsBuilder.fromPath(path)
                    .queryParam("OPERATION-NAME", "findItemsByProduct")
                    .queryParam("SERVICE-VERSION", apiVersion)
                    .queryParam("SECURITY-APPNAME", apiKey)
                    .queryParam("GLOBAL-ID", "EBAY-US")
                    .queryParam("RESPONSE-DATA-FORMAT", defaultDataFormat)
                    ...
                    .queryParam("affiliate.customId", affiliateCustomId)
                    .queryParam("outputSelector", "PictureURLLarge")
                    .queryParam("REST-PAYLOAD")
                    .queryParam("productId.@type", getIdTypeEbay(idType))
            ...
            return WebClient.builder()
                    ...
                    .accept(APPLICATION_JSON)
                    .retrieve()
                    .bodyToMono(EbayProductDetailResponse.class)
                    .flatMap(x -> buildResponse(x));
    ...
    private boolean isValidProductDetailResponse(EbayProductDetailResponse response) {
        var item = response.getFindItemsByProductResponse().get(0);
        if (item.getErrorMessage() != null && !item.getErrorMessage().isEmpty()) {
            item.getErrorMessage().forEach(x -> LOGGER.error(x.toString()));
            return false;
        }

        return item.getSearchResult() != null && !item.getSearchResult().isEmpty();
    }
    ...
    private Product toProduct(EbaySearchItem item) {
        return new Product(
                ...
                BigDecimal.valueOf(Double.valueOf(Optional.ofNullable(item.getSellingStatus().get(0).getConvertedCurrentPrice().get(0).getValue()).orElse("0"))),
